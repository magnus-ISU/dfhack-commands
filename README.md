...
| `tarrasque` | enableable+service | 🟡 first cut — REDESIGN PENDING | Keeps the world's **forgotten beasts** from going extinct. They're a finite pool (worldgen makes one per underground region), so once your fort kills the ones in the caverns it can reach, the attacks stop. tarrasque **revives dead forgotten beasts** — reusing DFHack's `full-heal -r` (un-kills the unit, resets the histfig's `died_year` to -1, refills blood / heals the body, logs a *revived* history event). Identifies FBs by `creature_id ~ FORGOTTEN_BEAST_<n>`. **The committed code is a FIRST CUT that revives dead beasts _in place_ up to a target count — it does NOT yet meet the requested design.** The full plan (and open problems) lives in a big comment block at the top of `tarrasque.lua`; in short, the redesign wants: revive on the **winter solstice (1 Obsidian)** with a **1/100-per-beast-per-year** roll (no target); revive **OUTSIDE the fortress** — ⚠️ **cavern tiles the fort can reach are NOT "outside"**, so placement is unsolved; **retain records** (death event + killer + revive event); work in **adventure mode** (world-level persistence, not site data); revive **off-map / no-unit** deaths too (needs a working spawn primitive — `wildlife-spawn.spawn` does **not** work); material randomization left out. Enabled by `magnus-scripts lovely` only, or standalone. **Not** auto-enabled (opt-in) |
