...
| `tarrasque` | enableable+service | ✅ done | Keeps the world's **forgotten beasts** from going extinct. They're a finite pool (worldgen makes one per underground region), so once your fort kills the ones in the caverns it can reach, the attacks stop. tarrasque **revives dead forgotten beasts** — reusing DFHack's `full-heal -r`, which un-kills the unit, resets the historical figure's `died_year` to -1, refills blood / heals the body, and logs a *revived* history event — so a slain beast comes back to life where it fell (a live unit in the caverns again + a living world figure). Identifies FBs by `creature_id ~ FORGOTTEN_BEAST_<n>`; only beasts whose unit still exists (ones the fort met) are revivable, so a few that died off-map in worldgen are skipped. `tarrasque` (status: alive/dead/revivable) / `tarrasque revive [N\|all]` / `tarrasque target <N>` / **`enable tarrasque`** = background service that tops the living count back up to the target (~monthly) whenever it falls. **Not** auto-enabled (it puts live beasts back in play — opt-in) |
