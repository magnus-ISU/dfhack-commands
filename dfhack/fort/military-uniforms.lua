...
gate refuses any order for a piece it cannot make (DF's manager does not check, and would
turn them out regardless), saying so once on the console. Specs already asking for such a
piece, on squad positions or in the owned templates, are re-pointed at the substitute on the
next cycle -- but only the ones standing empty; a soldier already wearing looted high boots
keeps them.
...
-- what the civilization can forge -- filled in further down, next to the forge gate; one
...
function CM.forgeable(d)
    local ok, r = pcall(function()
        return (d.armorlevel or 0) > 0 and d.props and d.props.flags and d.props.flags.METAL
    end)
    return ok and r or false
end

-- Subtype by name, among what the civilization can MAKE. A name the civ has no recipe for
-- (a civ without high boots) is answered with the best forgeable piece of the same family it
-- does have -- low boots -- and, when there is none, with nil, so the slot is left out of the
-- template. It used to fall back to "any subtype by name", which put a piece nobody could
-- forge on every uniform; the orders for it went to the manager and DF, which does not check
-- the civ's list at the forge, made them anyway.
--
-- Weapons keep the old fallback: a weapon the civ cannot forge is still bought, looted, or
-- brought by a migrant, and a squad with no weapon slot at all is worse than one waiting.
local function resolve_sub(vec, civ_set, name, weapon)
    ...
    for i = 0, #vec - 1 do if civ_set[i] and CM.forgeable(vec[i]) then return i end end
end
...
local function create_template(ent, spec, armour, wmat)
    ...
    add_to_slot(u, 6, IT.WEAPON, resolve_sub(R.weapons, wset, spec.weapon, true), 0, wmat)
...
local function create_civilian_uniform(ent, spec)
    ...
    add_to_slot(u, 6, IT.WEAPON, resolve_sub(R.weapons, setof(r.weapon_type), spec.weapon, true), 0, wmat)  -- steel axe / silver mace
    ent.uniforms:insert('#', u)
    ent.next_uniform_id = ent.next_uniform_id + 1
    return u
end
...
-- item_type -> the civilization's list of subtypes it has a recipe for, and the itemdef
-- vector those index
CM.RESOURCE_LIST = {
    [df.item_type.ARMOR]  = {'armor_type',  'armor'},
    [df.item_type.HELM]   = {'helm_type',   'helms'},
    [df.item_type.PANTS]  = {'pants_type',  'pants'},
    [df.item_type.GLOVES] = {'gloves_type', 'gloves'},
    [df.item_type.SHOES]  = {'shoes_type',  'shoes'},
    [df.item_type.SHIELD] = {'shield_type', 'shields'},
    [df.item_type.WEAPON] = {'weapon_type', 'weapons'},
}

function CM.subtype_name(item_type, subtype)
    local rl = CM.RESOURCE_LIST[item_type]
    local d = rl and df.global.world.raws.itemdefs[rl[2]][subtype]
    return d and d.name or (tostring(df.item_type[item_type]):lower() .. ' #' .. tostring(subtype))
end

-- can the civilization forge this subtype at all? Read from the fort entity's resource
-- lists, cached per session (they are fixed at worldgen).
function CM.can_make(item_type, subtype)
    local rl = CM.RESOURCE_LIST[item_type]
    if not rl or subtype < 0 then return true end          -- no list for it: no opinion
    if not CM.cache then
        CM.cache = {}
        local ent = fort_entity()
        if not ent then return true end
        for it, entry in pairs(CM.RESOURCE_LIST) do
            local set = setof(ent.resources[entry[1]])
            if it == df.item_type.WEAPON then
                for _, x in ipairs(ent.resources.digger_type) do set[x] = true end
            end
            CM.cache[it] = set
        end
    end
    return CM.cache[item_type][subtype] == true
end
CM.warned = {}

-- The forgeable substitute for a subtype the civ cannot make: the best piece of the same
-- family it CAN forge (high boot -> low boot), or nil when there is none.
function CM.substitute(item_type, subtype)
    local rl = CM.RESOURCE_LIST[item_type]
    if not rl or item_type == df.item_type.WEAPON then return nil end
    local vec = df.global.world.raws.itemdefs[rl[2]]
    for i = 0, #vec - 1 do
        if CM.can_make(item_type, i) and CM.forgeable(vec[i]) then return i end
    end
end

-- REPAIR: specs already asking for a piece the civ cannot make. Every squad position and
-- every owned template was written by this script from the "any subtype by name" rule
-- that is now gone, so they are the script's own mistake to correct, not a player choice:
-- each such spec is re-pointed at the makeable substitute of its family (a spec pinned to a
-- specific item is left alone -- it names a thing that exists). With no substitute the
-- spec stays as it is and the forge gate below keeps it from being ordered. Returns the
-- number of specs re-pointed, by "from -> to".
function CM.repair_specs()
    local fort = df.global.plotinfo.group_id
    local changed = {}
    local function fix(item_type, subtype)
        if subtype < 0 or CM.can_make(item_type, subtype) then return nil end
        local sub = CM.substitute(item_type, subtype)
        if not sub then return nil end
        local k = CM.subtype_name(item_type, subtype) .. ' -> ' .. CM.subtype_name(item_type, sub)
        changed[k] = (changed[k] or 0) + 1
        return sub
    end
    for s = 0, #df.global.world.squads.all - 1 do
        ...
        if sq.entity_id == fort then
            for p = 0, #sq.positions - 1 do
                for slot = 0, 6 do
                    for _, sp in ipairs(sq.positions[p].equipment.uniform[slot]) do
                        -- a spec already holding something (looted high boots, say) keeps
                        -- it: the repair is for the slots standing empty with no way to fill
                        if sp.item_type >= 0 and #sp.assigned == 0 then
                            local sub = fix(sp.item_type, sp.item_subtype)
                            if sub and not dry_run then sp.item_subtype = sub end
                        end
                    end
                end
            end
    ...
    if ent then
        for i = 0, #ent.uniforms - 1 do
            ...
            if is_owned_name(u.name) then
                for slot = 0, 6 do
                    local types, subs = u.uniform_item_types[slot], u.uniform_item_subtypes[slot]
                    for j = 0, #subs - 1 do
                        local sub = fix(types[j], subs[j])
                        if sub and not dry_run then subs[j] = sub end
                    end
                end
            end
        end
...
local function queue_one(key, r, n)
    ...
    -- and never a piece the civilization has no recipe for: the manager takes the order
    -- and the forge makes it regardless, which is how a fort without high boots on its
    -- list keeps turning them out. Said once per kind per session on the console.
    if not CM.can_make(r.item_type, r.subtype) then
        if not dry_run then drop_order(key) end        -- and take back one already queued
        if not CM.warned[key] then
            CM.warned[key] = true
            print(('military-uniforms: not ordering %s -- your civilization cannot make them')
                  :format(CM.subtype_name(r.item_type, r.subtype)))
        end
        note('  - would NOT order %s: the civilization cannot make them',
             CM.subtype_name(r.item_type, r.subtype))
        return
    end
    if dry_run then
...
local function run_cycle()
    ...
    for k, n in pairs(CM.repair_specs()) do
        print(('military-uniforms: %d uniform spec(s) asked for a piece the civilization cannot make; re-pointed %s'):format(n, k))
    end
