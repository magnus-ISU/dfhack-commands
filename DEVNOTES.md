...
| `forge-bars` | overlay | ✅ done | Forge "Add new task" material rows: paints the bar count over "(opens menu)" (`iron (40 bars)`, `steel (no bars)`, `, N in use`). Rows found by scraping the text grid for the material NAME before the "(" (matching the tail flickers: readTile returns the last frame, already carrying our paint); rescans on list change (length/first/selected signature) and every 30 frames; bars recounted once a second. Forge + magma forge only |
