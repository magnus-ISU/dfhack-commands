...
| `creature-description` | overlay | ✅ done | Shows the selected creature's description (bottom-left); great for forgotten beasts. The **Kills line is categorized** in fixed order — demons, angels, megabeasts (incl. FBs/titans), beasts (semi-megabeasts), **undead** (a kill group flagged zombie/ghostly/any undead variant counts here, trumping its race — a skeletal elf is undead, not an elf), goblins, dwarves, humans, elves, kobolds, the **largest animal species killed** (by adult body size; other animals fold into "other"), other — e.g. `310 kills: 1 angel, 2 beasts, 152 goblins, 22 humans, 50 elves, 5 giant elephants, 78 other`. Counts named victims (history events, classified by victim race) + anonymous kill groups; angels detected by DIVINE/ANGEL token (no caste flag exists), demons/megabeasts/beasts by caste flags. `kill_summary(unit)` is module-public |
