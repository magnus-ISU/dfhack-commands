...
| `cheatmine` | one-shot (armok) | ✅ done | **Instantly mines everything designated** — mining, channels, ramps, and carved up/down/up-down stair designations (via `dig-now`) — then instantly builds any planned **constructed staircase** (the hanging up/down staircases `dig-shapes` places in the air gaps of a staircase column, via `build-now` with summoned materials), so a dragged staircase through open space completes end to end. Other planned buildings are deliberately untouched |
